...
import os
import pdb
from pointpillars_ros.drone_pointpillars.utils import bbox3d2bevcorners, box_collision_test, read_points, \
...
def data_augment(CLASSES, data_root, data_dict, data_aug_config):
    ...
    #pos = data_dict['gt_bboxes_3d'][:, :3]
    #print(f"AFTER NOISE: POS: {pos}", flush=True)
    ...
    #pos = data_dict['gt_bboxes_3d'][:, :3]
    #print(f"AFTER FLIP: POS: {pos}", flush=True)
    ...
    #pos = data_dict['gt_bboxes_3d'][:, :3]
    #print(f"AFTER SCALE TRAN: POS: {pos}", flush=True)

    # 5. points range filter
    point_range = data_aug_config['point_range_filter']
    data_dict = point_range_filter(data_dict, point_range)

    if len(data_dict['pts']) == 0:
        print("Warning: No points left after point range filter!", flush=True)
    ...
    # position of objects
    #pos = data_dict['gt_bboxes_3d'][:, :3]
    #print(f"AFTER FILTER: POS: {pos}", flush=True)
    
    num_objects_after = len(data_dict['gt_bboxes_3d'])
    if num_objects_after == 0:
        print("Warning: No objects left after object range filter!", flush=True)
