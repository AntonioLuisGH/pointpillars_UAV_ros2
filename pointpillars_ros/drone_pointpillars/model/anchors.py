...
import numpy as np
import torch
from pointpillars_ros.drone_pointpillars.utils import limit_period, iou2d_nearest
...
class Anchors():
    ...
    def get_anchors(self, feature_map_size, anchor_range, anchor_size, rotations):
        ...
        # linspace(start, stop, Number of samples to generate, endpoint=True, retstep=False, dtype=None, axis=0, *, device=None)
        ...
        # we shift the centers by half a grid to get the center coordinates
        ...
        # meshgrid(*tensors, indexing='ij') returns coordinate matrices from coordinate vectors.
        # x_centers = 1 , 2
        # y_centers = 1 , 2
        # meshgrid = (1,1), (1,2), (2,1), (2,2)

        # The mesh grid has a size of (216, 248, 1, 2) in this case. Here, 216 corresponds to the y dimension (feature_map_size[0]), 248 corresponds to the x dimension (feature_map_size[1]), 1 corresponds to the z dimension (since we only have one layer in z), and 2 corresponds to the number of rotations.
        meshgrids = torch.meshgrid(x_centers, y_centers, z_centers, rotations)
        meshgrids = list(meshgrids) # convert the tuple into a list, so it can be changed.
        for i in range(len(meshgrids)): # append a trailing singleton dimension to each coordinate tensor. If meshgrid gave shape (X, Y, 1, R) each becomes (X, Y, 1, R, 1)
            meshgrids[i] = meshgrids[i][..., None] # [feature_map_size[1], feature_map_size[0], 1, 2, 1]
        
        # We reshape the anchor size to (1, 1, 1, 1, 3) and repeat it to match the feature map size and number of rotations (So we can concatenate it with the meshgrid tensors later)
        ...
        # Now we insert the anchor size tensor into the meshgrid list at index 3 (after x, y, z coordinates)
        ...
        return anchors.squeeze(0) # [feature_map_size[0], feature_map_size[1], 2, 7] [y_l, x_l, rotations, x, y, z, w, l, h, theta]
    ...
    def get_multi_anchors(self, feature_map_size):
        ...
        rotations: [0, 1.57]
        return: shape=(y_l, x_l, 3, 2, 7) # now (y_l, x_l, 1, 2, 7)
        '''
        device = feature_map_size.device
        ...
        return multi_anchors # (y_l, x_l, 1, 2, 7)
...
def anchor_target(batched_anchors, batched_gt_bboxes, batched_gt_labels, assigners, nclasses):
    ...
    for i in range(batch_size):
        ...
        # Loop for each of the anchor groups, we only have one group in this case.
        # anchors: (y_l, x_l, 1, 2, 7)
        d1, d2, d3, d4, d5 = anchors.size()   
        for j in range(n_assigners): # multi anchors groups
	        ...
	        # We skip the batches with no ground truth boxes.
            if gt_bboxes.numel() == 0:
                print("-------- Skiped batch with no gt_bboxes --------",flush=True)
                num = cur_anchors.size(0)
                device = cur_anchors.device

                assigned_gt_labels         = torch.full((num,), nclasses, dtype=torch.long, device=device)  # background idx
                assigned_gt_labels_weights = torch.ones(num, device=device)  # negatives count in cls loss

                assigned_gt_reg            = torch.zeros(num, 7, device=device)
                assigned_gt_reg_weights    = torch.zeros(num, device=device)  # no reg loss

                assigned_gt_dir            = torch.zeros(num, dtype=torch.long, device=device)
                assigned_gt_dir_weights    = torch.zeros(num, device=device)  # no dir loss
